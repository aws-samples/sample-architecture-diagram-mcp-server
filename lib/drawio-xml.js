...
// AWS4 official group styles, generated from the shared GROUP_VARIANTS table
// (single source of truth in @aws-live-diagram/core) so the .drawio path offers
// the SAME 12 container types as the interactive HTML — no drift.
import { GROUP_VARIANTS, resolveVariant, variantKey } from "@aws-live-diagram/core";
...
// Typed-connection palette — mirrors the interactive HTML (CustomEdge) so the
// .drawio path styles network/iam/event/data edges the same way.
const CONN_TYPES = {
  network: { stroke: "#4a90d9" },
  iam:     { stroke: "#DD344C", dashPattern: "6 4" },
  event:   { stroke: "#E7157B", dashPattern: "2 4" },
  data:    { stroke: "#3F8624" },
};

// Generate a service container (tint box) + centered 48x48 icon pair. The box
// size matches the shared layout engine's node geometry so edges/containers line
// up (defaults to 180x130 = DEFAULT_GEOMETRY.nodeW/nodeH).
function serviceCell(id, service, shape, category, x, y, label, parentId, w = 180, h = 130) {
  ...
  const iconX = Math.round((w - 48) / 2);
  const iconY = Math.round((h - 48) / 2 - 8); // nudge up; label sits below the icon
  ...
  const icon = `        <mxCell id="${svcId}" style="sketch=0;points=[[0,0,0],[0.25,0,0],[0.5,0,0],[0.75,0,0],[1,0,0],[0,1,0],[0.25,1,0],[0.5,1,0],[0.75,1,0],[1,1,0],[0,0.25,0],[0,0.5,0],[0,0.75,0],[1,0.25,0],[1,0.5,0],[1,0.75,0]];outlineConnect=0;fontColor=#232F3E;fillColor=${cat.fill};strokeColor=#ffffff;dashed=0;verticalLabelPosition=bottom;verticalAlign=top;align=center;html=1;fontSize=10;fontStyle=0;aspect=fixed;shape=mxgraph.aws4.resourceIcon;resIcon=mxgraph.aws4.${shape};fontFamily=Helvetica;shadow=1;" value="${val}" vertex="1" parent="${grpId}">
          <mxGeometry x="${iconX}" y="${iconY}" width="48" height="48" as="geometry" />
        </mxCell>`;

  return container + "\n" + icon;
}

// Generate edge with explicit exit/entry points. `type` (network/iam/event/data)
// drives colour + dash, matching the HTML; falls back to the neutral grey line.
function edgeCell(id, source, target, dashed, exitX, exitY, entryX, entryY, type, label) {
  const ex = exitX ?? 1; const ey = exitY ?? 0.5;
  const enx = entryX ?? 0; const eny = entryY ?? 0.5;
  const t = CONN_TYPES[type];
  const stroke = t ? t.stroke : "#545B64";
  // Explicit `dashed` flag wins; otherwise the type may carry its own dash pattern.
  const dashStr = dashed
    ? `dashed=1;strokeColor=${stroke};`
    : t?.dashPattern
      ? `dashed=1;dashPattern=${t.dashPattern};strokeColor=${stroke};`
      : `strokeColor=${stroke};`;
  const labelAttr = label ? ` value="${label}"` : "";
  return `        <mxCell id="${id}"${labelAttr} style="${EDGE_STYLE}${dashStr}exitX=${ex};exitY=${ey};exitDx=0;exitDy=0;entryX=${enx};entryY=${eny};entryDx=0;entryDy=0;" edge="1" parent="1" source="svc-${source}" target="svc-${target}">
          <mxGeometry relative="1" as="geometry" />
        </mxCell>`;
}

function groupCell(id, type, label, x, y, w, h, parentId) {
  // Resolve aliases (az → availability-zone, etc.) and fall back to the generic
  // 'group' style for anything unknown — never silently mislabel as aws-cloud.
  const style = GROUP_STYLES[variantKey(type)] || groupStyle(resolveVariant(type));
...
export function generateDrawio(title, subtitle, services, connections, groups, options = {}) {
  ...
  for (const c of connections) {
    cells.push(edgeCell(c.id, c.source, c.target, c.dashed, c.exitX, c.exitY, c.entryX, c.entryY, c.type, c.label));
  }
  ...
  if (steps && steps.length > 0) {
    ...
    // Legend panel — placed to the RIGHT of all content (dynamic), so it never
    // overlaps the diagram regardless of how wide the layout grows.
    let maxX = 0, minY = Infinity;
    for (const g of groups) { maxX = Math.max(maxX, g.x + g.w); minY = Math.min(minY, g.y); }
    for (const s of services) { maxX = Math.max(maxX, s.x + 180); minY = Math.min(minY, s.y); }
    if (users) { maxX = Math.max(maxX, users.x + 107); minY = Math.min(minY, users.y); }
    for (const e of (externals || [])) { maxX = Math.max(maxX, e.x + 130); minY = Math.min(minY, e.y); }
    const panelX = (maxX > 0 ? maxX : 1740) + 60;      // 60px gutter after the content
    const panelY = Number.isFinite(minY) ? Math.max(minY, 130) : 130;
    cells.push(legendPanel(steps, panelX, panelY));
  }
...
// Legend panel (right sidebar). panelX/panelY are computed by the caller from the
// real diagram bounds so the panel sits to the right of all content (never on top).
function legendPanel(steps, panelX = 1780, panelY = 130) {
