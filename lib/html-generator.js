...
// Group nodes reference these icons by variant (see GroupNode.tsx).
const GROUP_ICONS = [
  'AWS-Cloud_32.png',
...
export function generateHtml(title, subtitle, services, connections, options = {}) {
  ...
  const archData = JSON.stringify({ title, subtitle, services: resolved, connections, direction: options.direction || 'TB' });
