...
// A second view of the SAME system: who talks to whom, in order. Authored as a
// flat event list (like Mermaid's sequenceDiagram) so it is trivial to emit and
// to diff; the renderer animates it beat by beat (reveal + highlight) and boxes
// alt/opt/loop fragments from their explicit start/else/end markers.
export const sequenceParticipantSchema = z.object({
  id: z.string().describe("Short id referenced by every event's `from`/`to`/`over`."),
  label: langText.describe("Column header text. Use '\\n' for a second line (e.g. 'Lambda\\naft-seed-vault-rhbk')."),
  kind: z.enum(["actor", "participant"]).optional().describe("actor = a human/external role (drawn with a person glyph); participant (default) = a system/service box."),
  service: langText.optional().describe("Canonical AWS service name used to AUTO-RESOLVE this participant's icon (same resolution as a node's `service`). Omit when you pass `icon`."),
  icon: z.string().optional().describe("Explicit icon reference for the column header (e.g. 'tech-icons/vault.svg'). Overrides `service`."),
  pill: langText.optional().describe("Tiny qualifier under the header (e.g. 'on-premises', 'aft-management')."),
  color: z.string().optional().describe("Hex accent for this participant's header + lifeline."),
});

export const sequenceEventSchema = z.object({
  kind: z.enum(["message", "note", "fragment", "else", "end"]).optional().describe("message (default) = an arrow between participants; note = a boxed annotation over one or more lifelines; fragment = opens an alt/opt/loop box; else = a branch separator inside the open fragment; end = closes it."),
  from: z.string().optional().describe("Source participant id (message). Equal to `to` draws a self-call loop."),
  to: z.string().optional().describe("Target participant id (message)."),
  label: langText.optional().describe("Arrow label / note body / fragment title. Supports **bold**, `code` and '\\n'."),
  over: z.array(z.string()).optional().describe("Participant ids a `note` or `fragment` spans. Omit on a fragment to span every participant it contains."),
  dashed: z.boolean().optional().describe("Dashed arrow — the UML convention for a reply/return."),
  reply: z.boolean().optional().describe("Alias for dashed: marks the event as a response (dashed, open arrowhead)."),
  async: z.boolean().optional().describe("Open (half) arrowhead for an asynchronous send."),
  tone: toneEnum.optional().describe("Named accent for this beat: accent (default), info, success, warn (a gate), danger (an abort), neutral."),
  fragment: z.enum(["alt", "opt", "loop", "par", "critical"]).optional().describe("Fragment operator when kind='fragment' (default 'alt')."),
  condition: langText.optional().describe("Guard text shown in brackets on a fragment/else row, e.g. 'dispararVend = true'."),
  gate: z.boolean().optional().describe("Marks the step as a GATE that can abort the flow — badged and tinted amber."),
  note: langText.optional().describe("Extra commentary shown in the step's caption strip while this beat is active (not drawn on the canvas)."),
});

export const sequenceSchema = z.object({
  title: langText.optional().describe("Heading above the sequence canvas."),
  subtitle: langText.optional().describe("One-line context under the title."),
  autonumber: z.boolean().optional().describe("Number every message (1,2,3…) like Mermaid's autonumber. Default true."),
  participants: z.array(sequenceParticipantSchema).describe("Columns, in display order (left→right)."),
  events: z.array(sequenceEventSchema).describe("The flow, in order. Animated one beat at a time."),
  legend: z.array(z.object({
    label: langText,
    tone: toneEnum.optional(),
    color: z.string().optional(),
  })).optional().describe("Small legend under the canvas (e.g. 'GATE — can abort', 'keyless')."),
});
