...
export function StandaloneApp({ data }: Props) {
  ...
  const exportCode = useCallback((kind: 'iac' | 'calculator') => {
    ...
    // Both are MCP handoff payloads (JSON): IaC -> awslabs-iac-mcp, pricing -> aws-pricing-calculator-mcp.
    const code = kind === 'iac'
  ...
  return (
    <div className={`awsdiagram-root ${theme}`} data-theme={theme} style={{ width: '100vw', height: '100vh' }}>
      ...
      <div style={{ width: '100%', height: '100%', position: 'relative' }} onClick={(e) => {
        ...
        {codePanel && (() => {
          const titleLabel = codePanel.lang === 'iac' ? 'IaC → aws-iac-mcp' : 'Pricing → aws-pricing-calculator-mcp';
          const desc = codePanel.lang === 'iac'
            ...
            : (lang === 'pt' ? 'Payload para estimar custos via aws-pricing-calculator-mcp (sem credenciais AWS). Cole no agente.' : 'Payload to estimate costs via aws-pricing-calculator-mcp (no AWS credentials). Paste to the agent.');
          return (
